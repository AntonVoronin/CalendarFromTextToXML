...
public class BookFromSite {
	...
	public void DetectChapter() {
		...
		for (int i=1; i<TextPoSlovam.size(); i++) {
			...
			if (word.equals(":")) { //Прошлое слово - глава
				String textGlava = TextPoSlovam.get(i-1);
				if (textGlava.matches("c|C")) {
					TekGlava =  1003; //Специально для книги Esth C делаем номер главы -100
				}
				else {
			...
			if (word.equals(",") || word.equals(";")) { //Это конец отрывка
				if (!OtrivokIzDvuhChastey) { //придется заполнить и начало отрывка
			...
			if (word.equals("-")) { //Отрывок состоит из двух частей, и прошлое слово - начало отрывка
				OtrivokIzDvuhChastey = true;
				
				//Заполняем начало отрывка
				otrivok.chapterStart = TekGlava;
	...
	private String poluchitTextBezKnigi(String str) {
		...
		//TODO Проверка странных символов
...
//		}
	...
	public void detectTextKnigi(String textPredBook) {
		...
		//System.out.println("Текст книги \'" + book + "\'");
	...
	public void sortFragments() {
		//Определяем количество не пустых элементов, сортируем
		int i = 0;
		while(otr[i] != null) {
			i++;
		}
